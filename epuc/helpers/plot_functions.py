...
def plot_gaussian_nig_prediction_intervals(
    ...
    for i, ens_type in enumerate(results_dict.keys()):
        if ens_type not in  ["x_train", "y_targets"]: 
            ...
            if ens_type == "Normal":
                ...
                if plot_mean_params:
                    ax[i, 3].plot(results_dict[ens_type]["conf_bounds"]["param_0"]["mean"], label=r"$\mu$")
                    ax[i, 3].plot(results_dict[ens_type]["conf_bounds"]["param_1"]["mean"], label=r"$\sigma$")
                ...
                if plot_mean_params:
                    ax[i, 3].plot(results_dict[ens_type]["conf_bounds"]["param_0"]["mean"], label=r"$\gamma$")
                    ax[i, 3].plot(results_dict[ens_type]["conf_bounds"]["param_1"]["mean"], label=r"$\nu$")
                    ax[i, 3].plot(results_dict[ens_type]["conf_bounds"]["param_2"]["mean"], label=r"$\alpha$")
                    ax[i, 3].plot(results_dict[ens_type]["conf_bounds"]["param_3"]["mean"], label=r"$\beta$")
                    ax[i, 3].set_xlabel("epoch")
                    ax[i, 3].legend()
    ...
    for i, ens_type in enumerate(results_dict.keys()):
        if ens_type not in  ["x_train", "y_targets"]: 
            ...
            if ens_type == "Bernoulli":
                ...
                # plot mean output parameters if desired
                if plot_mean_params:
                    ax[i, 2].plot(results_dict[ens_type]["conf_bounds"]["param_0"]["mean"], label=r"$\theta$") # take mean because shape is [n_members, n_epochs]
                    ax[i, 2].set_xlabel("epoch")
                    ax[i, 2].legend()

            else:
                # plot predictions for mu ------------------------------------
            #  print(results_dict[ens_type]["mean_pred_p"].shape)
                ax[i, 0].plot(
                    x_eval,
                ...
                # plot mean output parameters if desired
                if plot_mean_params:
                    ax[i, 2].plot(results_dict[ens_type]["conf_bounds"]["param_0"]["mean"], label=r"$\alpha$") # take mean because shape is [n_members, n_epochs]
                    ax[i, 2].plot(results_dict[ens_type]["conf_bounds"]["param_1"]["mean"], label=r"$\beta$")
                    ax[i, 2].set_xlabel("epoch")
                    ax[i, 2].legend()
